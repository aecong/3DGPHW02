...
void CObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	std::uniform_real_distribution<float> uid(-500.f, 500.f);
	std::uniform_real_distribution<float> y(-250.f, 250.f);
	std::uniform_real_distribution<float> dir(-1.f, 1.f);
	std::uniform_real_distribution<float> FireTime(1.f, 5.f);
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		m_ppObjects[i]->SetCbvGPUDescriptorHandlePtr(d3dCbvGPUDescriptorNextHandle.ptr + (::gnCbvSrvDescriptorIncrementSize * i));
...
void CObjectsShader::ReleaseShaderVariables()
{
	...
	CStandardShader::ReleaseShaderVariables(); //어차피 아무것도 없음
}

void CObjectsShader::AnimateObjects(float fTimeElapsed)
{
	for (int i = 0; i < m_nObjects; i++) {
		CGameObject* pObject = m_ppObjects[i];

		// 현재 위치 가져오기
		XMFLOAT3 position = pObject->GetPosition();
		XMFLOAT3 direction = pObject->GetMovingDirection();

		position.x += direction.x * pObject->GetMovingSpeed() * fTimeElapsed;
		position.y += direction.y * pObject->GetMovingSpeed() * fTimeElapsed;
		position.z += direction.z * pObject->GetMovingSpeed() * fTimeElapsed;

		if (position.x < -750.0f || position.x > 750.0f) {
			direction.x *= -1; // X 방향 반전
		}
		if (position.y < -250.0f || position.y > 300.0f) {
			direction.y *= -1; // Y 방향 반전
		}
		if (position.z < -750.0f || position.z > 750.0f) {
			direction.z *= -1; // Z 방향 반전
		}

		pObject->SetMovingDirection(direction);

		position.x = std::clamp(position.x, -750.0f, 750.0f);
		position.y = std::clamp(position.y, -250.0f, 300.0f);
		position.z = std::clamp(position.z, -750.0f, 750.0f);
...
D3D12_SHADER_BYTECODE CTextureToScreenShader::CreateVertexShader(int nPipelineState)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSTextureToScreen", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CTextureToScreenShader::CreatePixelShader(int nPipelineState)
{
...
D3D12_DEPTH_STENCIL_DESC CTerrainShader::CreateDepthStencilState(int nPipelineState)
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	d3dDepthStencilDesc.DepthEnable = FALSE;
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
...
void CBillboardObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	ppGrassTextures[0] = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1, false);
	ppGrassTextures[0]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Grass01.dds", RESOURCE_TEXTURE2D, 0);
	ppGrassTextures[1] = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1, false);
	...
	ppFlowerTextures[0] = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1, false);
	ppFlowerTextures[0]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Flower01.dds", RESOURCE_TEXTURE2D, 0);
	ppFlowerTextures[1] = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1, false);
...
void CBillboardObjectsShader::ReleaseUploadBuffers()
{
	...
	CBOShader::ReleaseUploadBuffers();
}
...
void CBillboardObjectsShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, int nPipelineState)
{
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		if (m_ppObjects[j]) m_ppObjects[j]->LookAt(xmf3CameraPosition, XMFLOAT3(0.0f, 1.0f, 0.0f));
	}

	//CObjectsShader::Render(pd3dCommandList, pCamera);
	CBOShader::Render(pd3dCommandList, pCamera);
}
...
void CBOShader::AnimateObjects(float fDeltaTime)
{
	for (int j = 0; j < m_nObjects; j++)
	{
		m_ppObjects[j]->Animate(fDeltaTime);
	}
}

void CBOShader::ReleaseObjects()
{
	if (m_ppObjects)
	{
...
void CBOShader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nObjects, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CBOShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
	for (int j = 0; j < m_nObjects; j++)
	{
...
void CBOShader::ReleaseShaderVariables()
{
	if (m_pd3dcbGameObjects)
	{
		m_pd3dcbGameObjects->Unmap(0, NULL);
...
void CBOShader::ReleaseUploadBuffers()
{
	if (m_ppObjects)
	{
		for (int j = 0; j < m_nObjects; j++) if (m_ppObjects[j]) m_ppObjects[j]->ReleaseUploadBuffers();
...
void CBOShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, int nPipelineState)
{
