...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	//pd3dDescriptorRanges[5].BaseShaderRegister = 7; //t7: gtxtTexture[6]
	//pd3dDescriptorRanges[5].RegisterSpace = 0;
	//pd3dDescriptorRanges[5].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
...
void CStartScene::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	RECT rect = {
			static_cast<LONG>(0.575f * windowWidth), // 왼쪽 좌표
			static_cast<LONG>(0.375f * windowHeight), // 위쪽 좌표
			static_cast<LONG>((0.15f + 0.8f) * windowWidth), // 오른쪽 좌표
			static_cast<LONG>((0.15f + 0.4f) * windowHeight) // 아래쪽 좌표
	};
