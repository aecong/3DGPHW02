...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	switch (nMessageID)
	{
		...
		case WM_RBUTTONDOWN:
			...
			if (m_ppScenes[m_nScene]->b_NextSceneReady) {
	...
	switch (nMessageID)
	{
		case WM_KEYUP:
			switch (wParam)
			{
				...
				case VK_F2:
				case VK_F3:
					m_pCamera = m_pScene->m_pPlayer->ChangeCamera((DWORD)(wParam - VK_F1 + 1), m_GameTimer.GetTimeElapsed());
					break;
				case VK_F9:
...
void CGameFramework::BuildObjects()
{
	...
	if (m_nCurrentScene == 0) {
		...
		CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_ppScenes[0]->GetGraphicsRootSignature());
		pAirplanePlayer->SetPosition(XMFLOAT3(0.0f, 0.0f, 0.0f));
		m_ppScenes[0]->SetPlayer(pAirplanePlayer);
	}
	else if(m_nCurrentScene == 1) {
		m_ppScenes[1] = new CMenuScene();
		m_ppScenes[1]->BuildObjects(m_pd3dDevice, m_pd3dCommandList);

		CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_ppScenes[1]->GetGraphicsRootSignature());
		pAirplanePlayer->SetPosition(XMFLOAT3(0.0f, 0.0f, 0.0f));
		m_ppScenes[1]->SetPlayer(pAirplanePlayer);
	}
	else if(m_nCurrentScene == 2) {
		m_ppScenes[2] = new CMainScene();
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		DWORD dwDirection = 0;
		float speed = 2.5f;
		if (pKeysBuffer['F'] & 0xF0) speed = 10.f;
		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			if (cxDelta || cyDelta)
			{
				if (pKeysBuffer[VK_RBUTTON] & 0xF0)
					m_pScene->m_pPlayer->Rotate(cyDelta, 0.0f, -cxDelta);
				else
					m_pScene->m_pPlayer->Rotate(cyDelta, cxDelta, 0.0f);
			}
			if (dwDirection) m_pScene->m_pPlayer->Move(dwDirection, speed, true);
		}
	}
	m_pScene->m_pPlayer->Update(m_GameTimer.GetTimeElapsed());
}
